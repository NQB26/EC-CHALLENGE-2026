...
void motorControls::runMotor1(uint8_t pwm,uint8_t direction1,uint8_t direction2)
{
	...
	{	
		ledcSetup(0, 100, 8); // Set the PWM frequency (100 Hz in this example) and resolution (8 bits)
		ledcAttachPin(pwm, 0); // Attach the pin to the channel
		// ledcAttachPin(pwm,0);
		// ledcSetup(0,100,8);
	}
	if(pwmMotor1 > 0)
...
void motorControls::runMotor2(uint8_t pwm,uint8_t direction1,uint8_t direction2)
{
	...
	{	
		ledcSetup(1, 100, 8); // Set the PWM frequency (100 Hz in this example) and resolution (8 bits)	
		ledcAttachPin(pwm, 1); // Attach the pin to the channel
		// ledcAttachPin(pwm,1);
		// ledcSetup(1,100,8);
	}
	if(pwmMotor2 > 0)
...
void motorControls::runServo1(uint8_t pin)    //Attach Servo1 to channel 3
{
	...
	{	
		ledcSetup(3, 50, 16); // Set the PWM frequency (50 Hz for servos) and resolution (16 bits)
		ledcAttachPin(pin, 3); // Attach the pin to the channel
		// ledcAttachPin(pin,3);
		// ledcSetup(3,50,16);
	}
...
void motorControls::runServo2(uint8_t pin)	 //Attach Servo2 to channel 4
{
	...
	{	
		ledcSetup(4, 50, 16); // Set the PWM frequency (50 Hz for servos) and resolution (16 bits)
		ledcAttachPin(pin, 4); // Attach the pin to the channel
		// ledcAttachPin(pin,4);
		// ledcSetup(4,50,16);
	}
